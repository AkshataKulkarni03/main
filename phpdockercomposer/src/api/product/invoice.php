...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($invoiceId) && $action == 'payments') {
        ...
        echo "hititng payinvoice";
        $result = $obj->payInvoice($invoiceId,$data);
        if ($result) {
            http_response_code(200);
        ...
        $data = json_decode(file_get_contents('php://input'), true);
        $result = $obj->processOverdueInvoices($data);
        ...
        $result = $obj->createInvoice($invoiceDetails);
        if ($result) {
            http_response_code(200);
